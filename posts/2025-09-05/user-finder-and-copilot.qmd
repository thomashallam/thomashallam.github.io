...
In the last three years alone, we've had three more attempts at cracking the research repository. 

"Prototypes" were created by:

1. Normally, a specialist agency, who built a bespoke website (integrated with OpenAI), 
2. Microsoft, who built a PowerApps service (integrated with AzureAI), and 
3. NHSE UCD Team - we are now building a prototype with Microsoft Copilot Studio. 

Why so many iterations you ask? Well the tech/environment keeps changing, usually before we can rollout a thing that works.
...
We've had PowerPlatforms in the NHS for years, but usage never took off. As you can see, the frontend was very similar (albeit slightly clunky in PowerApps).
...
There are a significant number of Services, Products and teams across the NHS. Many of these teams have researchers, designers and product managers who require research insights to inform their Strategy and Roadmaps.
...
Only a few days into the build there was a bug, which Microsoft didn't fix, so it was broken. Apparently a new file processing capability would be landing soon, but for our pilot we had to look for more reliable options than Copilot as we had a limited time window to build something.
...
* Sometimes the agent would take many minutes to reply, other times it would be instant
...
There is much more to do around AI usage at NHS England, particularly improving policies, guidance and adoption levels. It feels like some colleagues are on the fence about AI, with many AI sceptics,ethicists and environmentalists (rightly) raising concerns too. 
